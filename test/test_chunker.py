...
@pytest.mark.parametrize(
    ...
    (
        ...
        [
            "csv", "TestTable", "/tmp", "test/csv/dir/*.csv", 1000, ";", "\\",
            '"', 20000
        ],
        [
            "csv",
...
@pytest.mark.parametrize(
    ...
    (
        ...
        [
            "parquet", "TestTable", "/tmp", "test/parquet/pyarrow_gzip.parquet",
            1000, 10
        ],
        [
            "parquet",
...
def test_parquet(fmt, name, temp_dir, filepath, chunksize, num_files):
    with create_chunks_dir(fmt, name, filepath, temp_dir,
                           chunksize) as chunks_dir:
        files = list(chunks_dir.iterdir())
        assert num_files == len(
            files), "this file should be split into 10 chunks"
        for file in files:
            df = pd.read_parquet(file)
            assert len(
                df
            ) == chunksize, f"chunk file should contain [{chunksize}] in this case"
    assert not chunks_dir.exists(), "Chunks Dir should be removed after usage"
